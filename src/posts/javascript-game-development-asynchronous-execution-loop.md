...
template: 'post.html'
---
One of the central core components of any game is the _game loop_. The
central piece of code that is responsible for balancing the execution of
logic and drawing operations as-well as user input.

Taking a simplistic naive approach, it could be written in the following form.

```javascript
class Game {
   constructor() {
      // Holds a time-stamp indicating when the last tick occurred.
...
condition](http://en.wikipedia.org/wiki/Race_condition) and the browser
...
We can solve this by introducing a fixed time-step.  We will keep drawing once
